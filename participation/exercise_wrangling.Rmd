...
```{r}
df<-select(gapminder, c(year,lifeExp,country))
```
...
##select only changes the singular variable, rename uses the entire data set
...
```{r}
gapminder %>%
  mutate(grossGDP=gdpPercap*pop,gdpBill=gdpPercap/100,signif(gdpBill, digits=3))
```
...
```{r}
gapminder %>%
  transmute(grossGDP=gdpPercap*pop,gdpBill=gdpPercap/100,signif(gdpBill, digits=3))
```
...
(c) Arrange by min life expectancy. ##trouble with arranging
...
##keep running into an error when trying to run the code below. it is something to do with the O_mean line, but I cannot trouble shoot.
```{r}
psych::bfi %>%
  as_tibble(rownames = ".id") %>% 
  ...
  mutate(A_mean = mean(c_across(A1:A5), na.rm = TRUE),
         O_mean = mean(c_across(O1:O5), na.rm = TRUE)) %>% 
  ungroup()
```
...
psych::bfi %>%
  as_tibble(rownames = ".id") %>% 
  select(.id, education, C1:C5, E1:E5, N1:N5) %>% 
  rowwise() %>% 
  mutate(C_mean = mean(c_across(C1:C5), na.rm = TRUE),
         E_mean = mean(c_across(E1:E5), na.rm = TRUE),
         N_mean = mean(c_across(N1:N5), na.rm = TRUE),
         sd_c = sd(c_across(C1:C5)),
          sd_E = sd(c_across(E1:E5)),
          sd_N = sd(c_across(N1:N5)),
         min_c=min(c_across(C1:C5)),
         min_e=min(c_across(E1:E5)),
         min_N=min(c_across(N1:N5)))%>%
ungroup()
```
...
psych::bfi %>%
  group_by(education) %>% 
  mutate(O_mean = mean(c_across(O1:O5), na.rm = TRUE),
         C_mean = mean(c_across(C1:C5), na.rm = TRUE),
         E_mean = mean(c_across(E1:E5), na.rm = TRUE),
         A_mean = mean(c_across(A1:A5), na.rm = TRUE),
         N_mean = mean(c_across(N1:N5), na.rm = TRUE)) %>% 
  ungroup()
```
