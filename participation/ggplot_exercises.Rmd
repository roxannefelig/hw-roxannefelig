...
- Change the following plot so that it shows _proportion_ on the y-axis, not count. HAVING ISSUES HERE. I set it to percentages but the percent values are very high
...
ggplot(mtcars2) +
  aes(cyl) +
  geom_bar(aes(color = transmission, fill=transmission), position="dodge")+
...
## in this, the points are on top of the lines.
```
__Exercise 3__: 
...
## yes, the lines show up because we have indicated what variables to use, we just have not asked ggplot to plot each specific data point.
```
...
## in this, we can't tell how each country differs, we just see an average of all of their regression slopes.
```
__Exercise 4__: 
