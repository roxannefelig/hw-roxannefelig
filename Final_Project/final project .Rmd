...
ie_clean<-ie_reverse %>% 
  ...
  relocate(Q28_13, .after= "Attn_check2") ##moved the three attn check var to the end so that i can more easily summarize across variables 
```
...
ie_full<-ie_clean %>%
  ...
  mutate(
    ...
    bod_shame=mean(c(BS1, BS2,BS3,BS4,BS5r,BS6, BS7r,BS8), na.rm = TRUE), ##couldn't just summarize across because the reverse coded items ended up at the end of the data set
...
cor(women_only, method = "pearson",use = "complete.obs")
```
...
cor(men_only, method = "pearson",use = "complete.obs")
```